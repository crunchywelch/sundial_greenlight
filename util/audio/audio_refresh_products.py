...
Usage:
    ...
    python util/audio/audio_refresh_products.py --sku SC-STD-BK-10-TS  # Check a specific SKU
"""
...
def fetch_all_products():
    """Fetch all products from audio Shopify store with qty and cost.
    ...
    try:
        ...
        while has_next_page:
            ...
            for edge in edges:
                ...
                product_type = product.get("productType", "") or "Audio Cable"
...
def main():
    parser = argparse.ArgumentParser(
        description="Refresh product data from audio Shopify store into SQLite"
    )
    parser.add_argument("--sku", help="Show data for a specific SKU after refresh")
    args = parser.parse_args()

    print("Refreshing products from audio Shopify store...")
    print()
    ...
    # Summary
    audio = conn.execute(
        "SELECT COUNT(*) FROM products WHERE is_wire = 0 AND product_type = 'Audio Cable'"
    ).fetchone()[0]
    total = conn.execute("SELECT COUNT(*) FROM products").fetchone()[0]
    print(f"  Audio cable products: {audio}")
