...
export async function action({ request }) {
  ...
  let created;
  try {
    created = await createLtdEdition({ slug, description });
  } catch (e) {
    if (e instanceof EditionValidationError || e instanceof EditionConflictError) {
      return json(
        { error: e.message, field: e.field, values: { slug, description } },
        { status: 400 }
      );
    }
    console.error("createLtdEdition failed:", e);
    return json(
      { error: `Database error: ${e.message}`, values: { slug, description } },
      { status: 500 }
    );
  }
  ...
  return (
    <div style={{ padding: "20px", maxWidth: "640px", margin: "0 auto", fontFamily: "system-ui, -apple-system, sans-serif" }}>
      ...
      <p style={{ color: "#666", fontSize: "13px", marginBottom: "20px" }}>
        Editions are tags that span all cable types. Cables registered against this edition carry their own series, length, connector, and other attributes.
      </p>
