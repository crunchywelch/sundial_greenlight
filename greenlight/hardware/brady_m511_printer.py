...
class BradyM511Printer(LabelPrinterInterface):
    ...
    def __init__(self, device_path: Optional[str] = None):
        ...
        Args:
            device_path: Bluetooth MAC address (defaults to known Brady M511)
        """
        self.device_path = device_path or BRADY_MAC
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            logger.info("Initializing Brady M511 printer via Bluetooth")
    ...
    async def _initialize_bluetooth_async(self) -> bool:
        ...
        try:
            logger.info(f"Connecting to Brady M511 at {self.device_path}")
            
            # Create client with tested timeout
            self.ble_client = self.BleakClient(self.device_path, timeout=20.0)
            
            # Connect (this was proven to work in our tests)
            ...
            logger.info("Brady M511 connected successfully")
...
class MockLabelPrinter(LabelPrinterInterface):
    """Mock label printer for testing without Brady hardware"""
...
def discover_brady_printers() -> List[Dict[str, str]]:
    """
    Discover Brady M511 printers
